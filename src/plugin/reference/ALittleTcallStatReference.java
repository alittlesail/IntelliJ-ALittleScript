...
public class ALittleTcallStatReference extends ALittleReference<ALittleTcallStat> {
    ...
    public List<ALittleReferenceUtil.GuessTypeInfo> guessTypes() throws ALittleReferenceUtil.ALittleReferenceException {
        ...
        if (valueStatList.isEmpty()) {
            throw new ALittleReferenceUtil.ALittleReferenceException(myElement, "pcall表达式不能没有参数");
        }
        ...
        if (guessInfo.type != ALittleReferenceUtil.GuessType.GT_FUNCTOR) {
            throw new ALittleReferenceUtil.ALittleReferenceException(valueStat, "pcall表达式第一个参数必须是一个函数");
        }
    ...
    public void checkError() throws ALittleReferenceUtil.ALittleReferenceException {
        ...
        if (valueStatList.size() - 1 > guessInfo.functorParamList.size()) {
            if (guessInfo.functorParamTail == null) {
                throw new ALittleReferenceUtil.ALittleReferenceException(myElement, "pcall表达式参数太多了");
            } else {
                throw new ALittleReferenceUtil.ALittleReferenceException(myElement, "pcall表达式参数太多了，即使被pcall的函数定义了参数占位符(...)也不行!");
            }
        }
