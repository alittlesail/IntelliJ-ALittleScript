...
public class ALittleParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // paths '(' TEXT_CONTENT? ')'
  public static boolean PathsValue(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "PathsValue")) return false;
    if (!nextTokenIs(b, PATHS)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, PATHS_VALUE, null);
    r = consumeTokens(b, 1, PATHS, LPAREN);
    p = r; // pin = 1
    r = r && report_error_(b, PathsValue_2(b, l + 1));
    r = p && consumeToken(b, RPAREN) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }
  ...
  // TEXT_CONTENT?
  private static boolean PathsValue_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "PathsValue_2")) return false;
    consumeToken(b, TEXT_CONTENT);
    return true;
  }
  ...
  /* ********************************************************** */
  // WrapValueStat | ConstValue | ReflectValue | PathsValue | PropertyValue | MethodParamTailDec | CoroutineStat
