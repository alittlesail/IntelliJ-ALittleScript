...
IMessageWriteFactory = Class(nil, "IMessageWriteFactory")
...
IMessageReadFactory = Class(nil, "IMessageReadFactory")
...
CreateArrayInfo = function(var_type)
	local invoke_info = {}
	invoke_info.sub_type = __sub(var_type, 6, -2)
	invoke_info.wfunc = PS_WriteArray
	invoke_info.rfunc = PS_ReadArray
	invoke_info.sub_info = CreateSubInfo(invoke_info.sub_type)
	return invoke_info
end
...
CreateMapInfo = function(var_type)
	...
	invoke_info.key_type = __sub(sub_type, 1, comma_index - 1)
	invoke_info.value_type = __sub(sub_type, comma_index + 1)
	invoke_info.wfunc = PS_WriteMap
	invoke_info.rfunc = PS_ReadMap
	invoke_info.key_info = CreateSubInfo(invoke_info.key_type)
	invoke_info.value_info = CreateSubInfo(invoke_info.value_type)
	return invoke_info
end
