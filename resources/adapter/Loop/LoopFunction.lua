...
function LoopFunction:SetTime(time)
	...
	if time < total_time then
		...
		local count = math.floor(float_count)
		self._accumulate_count = count
		self._accumulate_time = math.floor((float_count - count) * self._total_interval_time)
		return 0, false
	end
	self._accumulate_count = self._total_count
...
function LoopFunction:Start()
	A_LoopSystem:AddUpdater(self)
end

function LoopFunction:Close()
	A_LoopSystem:RemoveUpdater(self)
end
